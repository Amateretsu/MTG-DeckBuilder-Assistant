...
compatibility: Uses mtg-format-rules for format/deck-size rules (section 1), mtg-collection for owned-card data (section 3), mtg-card-taxonomy for functional-role categorization (section 4), mtg-consistency for land/color sanity checks (section 2), and mtg-edhrec for Commander synergy grounding (section 4). Works without any — see fallback notes per section — but more consistent with all installed.
---
